...
def _synchronized(method):
    """Serialize a BulletinStore method on the shared class-level I/O lock.

    The Web UI builds its own BulletinStore instance in the same process as
    the daemon, so the lock is class-level (shared by all instances) to
    serialize concurrent access to cognitive_bulletin.jsonl.
    """
...
class BulletinStore:
    ...
    # Shared across all instances in this process (daemon + UI).
    ...
    def _merge_from_disk(self) -> None:
        ...
        Guards against another in-process BulletinStore (the Web UI builds
        its own) having appended entries our cache predates.
        """
            ...
            with open(self._path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if not line:
                        continue
                    try:
                        entry = BulletinEntry.from_dict(json.loads(line))
                    except (json.JSONDecodeError, KeyError, ValueError):
                        continue
                    cached = self._entries.get(entry.entry_id)
                    if cached is None or entry.updated_at >= cached.updated_at:
                        self._entries[entry.entry_id] = entry
        except OSError as e:
            logger.error(f"[BULLETIN] Cannot merge {self._path}: {e}")
    ...
    @_synchronized
    def tag_escalated(self, entry_id: str, goal_id: str) -> bool:
        """Mark entry as escalated to a PROPOSED goal (Most #1 BulletinEscalator).

        Sets metadata.escalated_to_goal and updated_at, persists. Status stays
        OPEN — the entry will resolve when the goal is accepted/rejected.
        ...
        if entry is None:
            return False
        entry.metadata["escalated_to_goal"] = goal_id
        entry.updated_at = time.time()
        self._append(entry)
        logger.info(
            f"[BULLETIN] Escalated: {entry_id} -> goal {goal_id}"
        )
        return True
    ...
    def compact(self) -> None:
        ...
        Merges records from disk first (newer updated_at wins) so a rewrite
        from our cache never drops entries another in-process instance
        appended, and writes atomically via a temp file + replace.
        """
        self._ensure_loaded()
        assert self._entries is not None
